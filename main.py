            ...
            if st.session_state.get('show_date_picker'):
                with st.container(border=True):
                    ...
                    if not versions.empty:
                        v_choisie = st.selectbox("Versions existantes", versions['Mois'].tolist())
                ...
                if not versions_du_mois.empty:
                    ...
                    if cur_v == max_v:
                        ...
                        if st.session_state.get('ask_lock_pwd'):
                            with st.container(border=True):
                                pwd_bulletin = st.text_input("Entrez le PASSWORD (MODIFY)", type="password")
    ...
    with st.container(border=True):
        ...
        if st.button("💾 SAUVEGARDER CETTE VERSION", use_container_width=True):
            ...
            if not doublon_exact.empty:
                ...
                exist_versions = df_hist[(df_hist['Utilisateur'] == current_user) & (df_hist['Mois'].str.startswith(base_combinee))]
            ...
            if not data_user.empty:
                ...
                # --- 🟠 INTERFACE 2 : GESTION DES DÉPENSES ---
                    ...
                    with c1:
                        ...
                        st.markdown(f"""<div style="background-color:#0d47a1; color:white; padding:10px; border-radius:5px; border-left:5px solid #1565c0;">
                    ...
                    with c2:
                        ...
                        st.markdown(f"""<div style="background-color:#1b5e20; color:white; padding:10px; border-radius:5px; border-left:5px solid #2e7d32;">
